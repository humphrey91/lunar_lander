...
sides: number
  constructor(position: p5.Vector,id: number, distances: p5.Vector[],scale, p5: p5) {
    super(position.x,position.y,id, distances,scale, p5)
    this.sides = 3
  }
...
function createTriangle(x1,y1,x2,y2,x3,y3,id,scale,p5) {
  let coordinates: number[] = [x1,y1,x2,y2,x3,y3]
  ...
  return new Triangle(position, id, distances,scale,p5)
}
